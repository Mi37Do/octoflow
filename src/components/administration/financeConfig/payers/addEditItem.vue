...
watch(() => useWidget.addEditType.open, () => {
  if (useWidget.addEditType.add) {
    ...
    Object.assign(itemToAdd, {
      code: useAdministration.focusedPayer.code,
      designation: useAdministration.focusedPayer.designation,
      description: useAdministration.focusedPayer.description
    })
    loading.value = false
  }
})
...
const addEditItem = async () => {
  ...
  try {
    if (useWidget.addEditType.add) {
      ...
      let response = await axios.patch(`/api/Banque/${useAdministration.focusedPayer.id}/`, itemToAdd)
    }
