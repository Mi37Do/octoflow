          ...
          <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
            ...
            <DialogPanel
              ...
              <DialogTitle as="h3" class="pixa-title truncate">
                {{ useWidget.addEditType.add ? t('translation.add') + ' ' + t('translation.bank') :
                  t('translation.edit') + ':' +
                  useAdministration.focusedBank.designation }}
              </DialogTitle>
...
watch(() => useWidget.addEditType.open, () => {
  if (useWidget.addEditType.add) {
    ...
    Object.assign(itemToAdd, {
      code: useAdministration.focusedBank.code,
      designation: useAdministration.focusedBank.designation,
      description: useAdministration.focusedBank.description
    })
    loading.value = false
  }
})
...
const addEditItem = async () => {
  ...
  try {
    if (useWidget.addEditType.add) {
      ...
      let response = await axios.post(`/api/Banque/`, itemToAdd)
    } else {
      let response = await axios.patch(`/api/Banque/${useAdministration.focusedBank.id}/`, itemToAdd)
    }
