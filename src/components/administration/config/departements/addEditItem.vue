          ...
          <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
            ...
            <DialogPanel
              ...
              <DialogTitle as="h3" class="pixa-title truncate">
                {{ useWidget.addEditType.add ? t('translation.add') + ' ' + t('translation.departement') :
                  t('translation.edit') + ':' +
                  useAdministration.focusedDepartement.designation }}
              </DialogTitle>
...
watch(() => useWidget.addEditType.open, () => {
  if (useWidget.addEditType.add) {
    ...
    Object.assign(itemToAdd, {
      designation: useAdministration.focusedDepartement.designation,
      description: useAdministration.focusedDepartement.description
    })
    loading.value = false
  }
})
...
const addEditItem = async () => {
  ...
  try {
    if (useWidget.addEditType.add) {
      ...
      let response = await axios.post(`/api/Departements/`, itemToAdd)
    } else {
      let response = await axios.patch(`/api/Departements/${useAdministration.focusedDepartement.id}/`, itemToAdd)
    }
