          ...
          <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
            ...
            <DialogPanel
              ...
              <DialogTitle as="h3" class="pixa-title truncate">
                {{ useWidget.addEditType.add ? t('translation.add') + ' ' + t('translation.type') :
                  t('translation.edit') + ':' +
                  useProduct.focusedType.designation }}
              </DialogTitle>
...
watch(() => useWidget.addEditType.open, () => {
  if (useWidget.addEditType.add) {
    ...
    Object.assign(itemToAdd, {
      code: useProduct.focusedType.code,
      designation: useProduct.focusedType.designation
    })
    loading.value = false
  }
})
...
const addEditItem = async () => {
  ...
  try {
    if (useWidget.addEditType.add) {
      ...
      let response = await axios.post(`/api/Type/`, itemToAdd)
      console.log(response.data)
    } else {
      let response = await axios.patch(`/api/Type/${useProduct.focusedType.id}/`, itemToAdd)
      console.log(response.data)
    }
