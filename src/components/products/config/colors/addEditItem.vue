          ...
          <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
            ...
            <DialogPanel
              ...
              <DialogTitle as="h3" class="pixa-title truncate">
                {{ useWidget.addEditType.add ? t('translation.add') + ' ' + t('translation.color') :
                  t('translation.edit') + ':' +
                  useProduct.focusedColor.designation }}
              </DialogTitle>
...
const addEditItem = async () => {
  ...
  try {
    if (useWidget.addEditType.add) {
      ...
      let response = await axios.patch(`/api/Couleur/${useProduct.focusedColor.id}/`, itemToAdd)
    }
