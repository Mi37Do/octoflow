          ...
          <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
            ...
            <DialogPanel
              ...
              <DialogTitle as="h3" class="pixa-title truncate">
                {{ useWidget.addEditType.add ? t('translation.add') + ' ' + t('translation.packing') :
                  t('translation.edit') + ':' +
                  useProduct.focusedPacking.designation }}
              </DialogTitle>
...
watch(() => useWidget.addEditType.open, () => {
  if (useWidget.addEditType.add) {
    ...
    Object.assign(itemToAdd, {
      code: useProduct.focusedPacking.code,
      designation: useProduct.focusedPacking.designation
    })
    loading.value = false
  }
})
...
const addEditItem = async () => {
  ...
  try {
    if (useWidget.addEditType.add) {
      ...
      let response = await axios.post(`/api/Couleur/`, itemToAdd)
    } else {
      let response = await axios.patch(`/api/Couleur/${useProduct.focusedPacking.id}/`, itemToAdd)
    }
