    ...
    <div class="w-full h-10 flex items-center justify-between">
      ...
      <div class="flex gap-2">
        ...
        <button @click="() => {
          ...
          <Plus class="w-4" /> {{ t('translation.add') }} {{ t('translation.packing') }}
        </button>
      </div>
    </div>


    <commun-table v-if="useProduct.packings.length > 0" :columns="columns" :footer="true" @divsWidths="(values) => {
      divsWidths = values
    }">
      <template #table_content>
        <item-data v-for="item in useProduct.packings" :key="item.id" :item="item" :divsWidths="divsWidths" />
      </template>

      <template #table_footer_right>
        <table-elements :elements="useProduct.packings.length" />
      </template>
...
import addEditItem from '@/components/products/config/packing/addEditItem.vue';
...
import itemData from '@/components/products/config/packing/itemData.vue';
import { useProductStore } from '@/stores/products';
import optionsModal from '@/components/products/config/packing/optionsModal.vue';
...
const onDeleteItem = async (id) => {
  try {
    let response = await axios.delete(`/api/Conditionement/${id}/`)
    await useProduct.getPackings()
