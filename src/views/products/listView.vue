  ...
  <div class="w-full h-full flex flex-col">
    ...
    <div class="w-full flex-1 overflow-hidden">
      ...
      <div v-else class="w-full h-full flex flex-col gap-3 pb-3 px-3">
        ...
        <commun-table v-if="useProduct.products.length > 0" :columns="columns" :footer="true" @divsWidths="(values) => {
          divsWidths = values
        }">
          <template #table_content>
            <item-data v-for="item in useProduct.products" :key="item.id" :item="item" :divsWidths="divsWidths"
          ...
          <template #table_footer_right>
            <table-elements :elements="useProduct.products.length" />
          </template>
...
import addEditItem from '@/components/products/list/addEditItem.vue';
...
import itemData from '@/components/products/list/itemData.vue';
...
const onDeleteItem = async (id) => {
  try {
    let response = await axios.delete(`/api/Produit/${id}/`)
