    ...
    <div class="w-full h-10 flex items-center justify-between">
      ...
      <div class="flex gap-2">
        ...
        <button @click="() => {
          ...
          <Plus class="w-4" /> {{ t('translation.add') }} {{ t('translation.bank') }}
        </button>
      </div>
    </div>


    <commun-table v-if="useAdministration.banks.length > 0" :columns="columns" :footer="true" @divsWidths="(values) => {
      divsWidths = values
    }">
      <template #table_content>
        <item-data v-for="item in useAdministration.banks" :key="item.id" :item="item" :divsWidths="divsWidths" />
      </template>

      <template #table_footer_right>
        <table-elements :elements="useAdministration.banks.length" />
      </template>
...
import addEditItem from '@/components/administration/financeConfig/banks/addEditItem.vue';
...
import itemData from '@/components/administration/financeConfig/banks/itemData.vue';
import optionsModal from '@/components/administration/financeConfig/banks/optionsModal.vue';
...
onMounted(async () => {
  try {
    await useAdministration.getBanks()
    loading.value = false
  } catch (error) {
    console.error(error)
    loading.value = true
...
const onDeleteItem = async (id) => {
  try {
    let response = await axios.delete(`/api/Banque/${id}/`)
    await useAdministration.getBanks()
