  ...
  <div v-else class="w-full h-full flex flex-col gap-3 pb-3 px-3">
    ...
    <div class="w-full h-10 flex items-center justify-between">
      ...
      <div class="flex gap-2">
        ...
        <router-link :to="{ name: 'administration-config-payers-add-view' }" class="btn btn-primary">
          <Plus class="w-4" /> {{ t('translation.add') }} {{ t('translation.payer') }}
        </router-link>
      </div>
    </div>


    <commun-table v-if="useAdministration.payers.length > 0" :columns="columns" :footer="true" @divsWidths="(values) => {
      divsWidths = values
    }">
      <template #table_content>
        <item-data v-for="item in useAdministration.payers" :key="item.id" :item="item" :divsWidths="divsWidths" />
      </template>

      <template #table_footer_right>
        <table-elements :elements="useAdministration.payers.length" />
      </template>
...
import addEditItem from '@/components/administration/financeConfig/payers/addEditItem.vue';
...
import itemData from '@/components/administration/financeConfig/payers/itemData.vue';
import optionsModal from '@/components/administration/financeConfig/payers/optionsModal.vue';
...
const onDeleteItem = async (id) => {
  try {
    let response = await axios.delete(`/api/Payeur/${id}/`)
    await useAdministration.getPayers()
