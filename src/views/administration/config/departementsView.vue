<template>
  ...
  <div v-else class="w-full h-full flex flex-col gap-3 pb-3 px-3">
    ...
    <commun-table v-if="useAdministration.departements.length > 0" :columns="columns" :footer="true" @divsWidths="(values) => {
      divsWidths = values
    }">
      <template #table_content>
        <item-data v-for="item in useAdministration.departements" :key="item.id" :item="item"
          :divsWidths="divsWidths" />
      </template>

      <template #table_footer_right>
        <table-elements :elements="useAdministration.departements.length" />
      </template>
...
import addEditItem from '@/components/administration/config/departements/addEditItem.vue';
...
import itemData from '@/components/administration/config/departements/itemData.vue';
import optionsModal from '@/components/administration/config/departements/optionsModal.vue';
...
onMounted(async () => {
  try {
    await useAdministration.getDepartements()
    loading.value = false
  } catch (error) {
    console.error(error)
    loading.value = true
...
const onDeleteItem = async (id) => {
  try {
    let response = await axios.delete(`/api/Departements/${id}/`)
    await useAdministration.getDepartements()
